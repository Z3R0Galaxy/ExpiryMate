...
## 21/8/26 (Auth redirect URL fix — again)

**Prompt:** "when a new user signs in, they are propmeted to verify their email. they get sent a link to click. the link takes them to a 'local host' site and obvisly dosnt work. pleae fix."

**Response:** this is the same class of bug already fixed once on 18/8/26 (see `decisions.md`, "Auth redirect URL fix") by pointing the Supabase project's dashboard "Site URL" at the live Vercel domain — evidently that dashboard setting drifted back to `localhost` at some point since, or was never durably saved, since `Auth.tsx`'s `signUp` call never specified its own redirect and so always inherited whatever the dashboard default was. Checked the connector registry for a Supabase MCP connector first (`SearchMcpRegistry`/`ListConnectors`, keyword "supabase") in case this could be fixed directly — none is connected, so there's no way to touch the Supabase dashboard from this session; the dashboard-only fix from 18/8/26 isn't repeatable from here. Made the fix at the code level instead, which doesn't depend on that dashboard setting staying correct: `signUp` now passes `options: { emailRedirectTo: window.location.origin }`, so the confirmation link always points at wherever the app is actually being run from (the deployed site in production, `localhost` correctly if someone's testing sign-up from a local dev server) rather than a separately-configured default that can silently drift. Also told the user directly (not something I can fix myself) that Supabase still requires that exact origin to be present in the project's Auth → URL Configuration → **Redirect URLs** allow-list — if it's missing there, Supabase silently falls back to the Site URL regardless of what `emailRedirectTo` asks for, so the dashboard's Site URL should still be checked and the Redirect URLs list should include the Vercel domain (and `localhost:5173` for local testing) for this fix to fully take effect.

**Verified before accepting:** `tsc -b --force` and `eslint src`, both run directly on the user's machine via the device bridge, passed clean. Not browser-tested against a real email confirmation flow — doing that would need a live sign-up, which wasn't run as part of this fix.

**Not yet done:** the user checking their Supabase project's Auth → URL Configuration (Site URL + Redirect URLs) to confirm the Vercel domain is actually listed there, and a real sign-up test to confirm the confirmation link now lands correctly.
...
## 22/8/26 (Feedback Sprint 2 — a second round of feedback, ten items)

**Prompt:** "Ok so I have gotten a second round of feedback so lets make a new sprint kinda thing and commit after each change," followed by a numbered list of ten items: (1) the "opened" date picker wrongly rejected today's date; (2) redesign the "opened?" control as a label + checkbox, prompting for a date only once checked; (3) the bar/donut chart still showed a sliver for the (always-zero) "Unsafe" status; (4) add a "Fresh" stat tile to the top cards on every dashboard; (5) add a Frozen/microwavable-meals category, wired into the expiry algorithm; (6) block saving an item into an unsafe storage spot instead of letting it through with a warning; (7) remove "Unsafe" from the dashboards and list filter entirely, now that it's unreachable; (8) the add-item FAB should slide open on hover to reveal "Add item"; (9) clicking a chart segment should navigate the same way its matching stat tile does; (10) on phone, the donut's legend should sit beside the ring rather than below it.

**Response:** ten independent fixes, committed one at a time per the user's explicit instruction ("commit after each change") rather than as one batch — see the ten `Feedback Sprint 2 (N/10): ...` commits in git log for the full diff of each.

1. **False "date must not be in the future"** — root cause was a timezone mismatch, not a real future date: the future-date check built a `Date` from the plain `YYYY-MM-DD` string (parsed as UTC midnight) and compared it against local midnight, so for part of every day in Sydney (UTC+10) "today" itself read as in the future. Added a shared `todayLocal()` helper (`validateItem.ts`) that reads the browser's local calendar date directly, and changed the check to a plain string comparison (`date_opened > todayLocal()`), used consistently in `AddItemForm.tsx` and `ItemDetailModal.tsx`.
2. **"Opened?" as label + checkbox** — replaced the previous stacked layout with one row (`.opened-toggle`): the "Opened?" text and its checkbox side by side; the date field only appears once checked, and unchecking clears it (`AddItemForm.tsx`, `ItemDetailModal.tsx`, `App.css`).
3. **Donut drawing a dot for a 0-count status** — an SVG rendering quirk, not a data bug: `strokeLinecap="round"` on a zero-length arc still renders, because two round end-caps on a degenerate segment combine into a visible dot at that segment's angle. Fixed by filtering to only positive-value segments before rendering ring arcs in `Donut.tsx`; the legend row for a 0-count status is untouched, since showing "0" there in plain text is correct.
4. **"Fresh" stat tile** — added between "Total items" and "Expiring soon" on both `Dashboard.tsx` and `PlaceDashboard.tsx`, reusing the same `stat-tile`/`toggle()` pattern as the existing tiles, plus a `.stat-fresh` colour rule in `App.css`.
5. **Frozen Meals category** — added end-to-end: the `FoodCategory` union (`useItems.ts`), a Postgres migration adding the enum value (`supabase/migrations/20260822000000_add_frozen_meals_category.sql` — **not yet run against the live database**, see below), the category dropdown in both add/edit forms and `ItemList.tsx`'s filter, a `guessCategory.ts` keyword rule (ordered before the generic "Frozen" rule, since first match wins), a dedicated icon in `icons.tsx`, and a new `evalFrozenMeals()` rule in `adjustedExpiry.ts` — unopened must be in the freezer, opened must be in the fridge (not freezer, not pantry) and then behaves like a 3-day leftover.
6. **Block unsafe saves instead of warning** — new shared `checkStorageSafety()` helper (`validateItem.ts`, wrapping the existing `getAdjustedExpiry()`) is now called from both `AddItemForm.tsx`'s submit handler and `useItemDetail.ts`'s `saveEdit()`, right after the existing form validation; if the combination is unsafe, the save is refused and the existing warning message (see item 7 below) is shown as the form error instead of being saved anyway.
7. **Remove "Unsafe" from dashboards/list** — since item 6 makes an unsafe item unreachable going forward, removed the "Unsafe" stat tile from both dashboards, the `warning` segment from the status donut, and the "Unsafe" option from `ItemList.tsx`'s status filter. Left the underlying `BadgeStatus`/`warning` badge-rendering machinery in place as a defensive fallback for any pre-existing item that predates this change. Also rewrote every `warn(...)` message in `adjustedExpiry.ts` to name the correct storage location explicitly (e.g. "Store it in the fridge instead"), since those messages are now what a blocked save actually shows the user, not just an after-the-fact label.
8. **FAB hover-reveal animation** — restructured the FAB's markup into an icon span plus a label span (`App.tsx`) and rewrote `.fab`/`.fab-icon`/`.fab-label` in `App.css` as a `max-width`/`opacity` transition (chosen over animating `grid-template-columns` for reliability), with `:focus-visible` given the same reveal as `:hover` for keyboard users.
9. **Clicking a chart segment navigates** — added an optional `onSegmentClick` prop to both `Donut.tsx` and `BarChart.tsx`, wired to the ring arc/bar and to the legend row, with full keyboard support (`role="button"`, `tabIndex`, Enter/Space). `Dashboard.tsx` wires the status donut to the same `toggle()` its stat tiles call, and the location bar chart to `onNavigate('place-dashboard', ...)`, landing on that place's `PlaceDashboard` — the same destination the sidebar's Fridge/Freezer/Pantry links already use.
10. **Donut legend beside the ring on mobile** — the `@media (max-width: 560px)` breakpoint used to force `.donut-row` into a column, stacking the legend below the ring. Removed that override so it keeps the base row layout, just with a tighter gap and `justify-content: flex-start` (instead of desktop's `space-between`) so ring and legend sit snugly together rather than stretched across the full mobile card width.

**Verified before accepting:** every item was checked individually with `tsc -b --force` and `eslint src`, run directly on the user's machine via the device bridge, before being committed — all ten passed clean.

**Not yet done:** the Frozen Meals migration (`supabase/migrations/20260822000000_add_frozen_meals_category.sql`) needs to be run manually in the Supabase SQL Editor before that category actually works against the live database — it can't be applied from this session. All ten commits are local-only (`git push` is blocked from this sandbox, see earlier entries); the user still needs to run `git push origin main`. And, as with the animation work above, the FAB hover-reveal (item 8) and the mobile legend layout (item 10) are both things that read differently live in a browser than from source — worth the user's own look before considering them fully settled.

## 22/8/26 (Supabase SQL Editor query — delete existing unsafe items)

**Prompt:** "can we reesed the database to remove all the unsafe tags too?"

**Response:** clarified first, since "Unsafe" isn't a stored column — `computeStatusInfo()` (`itemStatus.ts`) derives it live from `getAdjustedExpiry()` (`adjustedExpiry.ts`), which returns `safe: false` for specific category/storage_location/opened-state combinations. So there was nothing to "clear" in the database directly; asked whether the goal was to move existing unsafe items to a safe storage location, delete them, or wipe the whole database and reseed. The user chose delete. Wrote a preview `select` plus a matching `delete` for the Supabase SQL Editor, with the `where` clause hand-built to mirror `getAdjustedExpiry()`'s unsafe branches exactly (Dairy/Meat/Seafood/Leftovers unsafe in Pantry; Eggs unsafe in Freezer or opened-in-Pantry; Beverages/Condiments unsafe — "not recommended" — in Freezer; Frozen unsafe anywhere but Freezer; Frozen Meals unsafe if sealed outside the freezer or opened in the freezer/pantry) — so the query deletes exactly the rows the app itself would currently badge "Unsafe," not a hand-guessed approximation. Given directly in chat for the user to run manually in the Supabase SQL Editor; no code changed, so nothing to commit.

**Not yet done:** the user running the query against the live database, and deciding whether to scope it to their own `user_id` or run it across all accounts (given as written).

## 22/8/26 (Rename "Frozen Meals" to "Microwave Meals")

**Prompt:** "can we chnage the name of "frozen meals" to "microwave meals""

**Response:** checked first whether the Feedback Sprint 2 migration adding this category had already been run against the live database, since that changes what "rename" actually needs to do — the user confirmed it had. Wrote a new migration, `20260822010000_rename_frozen_meals_to_microwave_meals.sql`, using `alter type food_category rename value 'Frozen Meals' to 'Microwave Meals'` rather than adding a second enum value and backfilling every row's `category` column — the rename relabels the existing value in place, so items already saved under the old name carry over with nothing left to backfill. Renamed every matching code reference to keep the app consistent with the new label: the `FoodCategory` type (`useItems.ts`), all three category dropdowns (`AddItemForm.tsx`, `ItemDetailModal.tsx`, `ItemList.tsx`), the `guessCategory.ts` rule, the icon lookup (`icons.tsx`), and `adjustedExpiry.ts`'s switch case and its `evalFrozenMeals` function (renamed to `evalMicrowaveMeals`). Left the original `add value` migration and the Feedback Sprint 2 log entries above alone, since they're a record of what actually shipped at the time, not something to rewrite after the fact.

**Verified before accepting:** `tsc -b --force` and `eslint src`, both run directly on the user's machine via the device bridge, passed clean.

**Not yet done:** running the new rename migration in the Supabase SQL Editor — until then, the live database's enum still says "Frozen Meals" while the app's code now says "Microwave Meals," so any item using this category will fail to save until the migration runs. `git push origin main` for this commit.

## 22/8/26 (Documentation catch-up: is Feedback Sprint 2 documented everywhere?)

**Prompt:** "is this revision sprint documented in all the relevnt docs?"

**Response:** audited every doc the project treats as a living reference, not just `ai-use-log.md`/`decisions.md` (which were already current). Found three real gaps: `docs/09-iteration-log.md` had no entry at all past "UI feedback pass seven bug fix," despite `10-feedback-sprint.md` explicitly stating every concrete Feedback Sprint step gets one there; `CLAUDE.md`'s Key Components/Current State/Database Schema sections and `docs/04-data-model.md`'s category list/migrations table hadn't been updated since the same point; and `docs/02-requirements.md`'s unsafe-combination requirement still described the old warn-and-allow behaviour rather than Feedback Sprint 2's new block-on-save behaviour. Backfilled all four files. No app code changed.

**Verified before accepting:** `tsc -b --force`/`eslint src` run (a no-op check, since only docs changed) — both passed clean.

## 22/8/26 (Status-breakdown donut: closing the empty-card gap)

**Prompt:** two screenshots of the dashboard's "Status breakdown" card at a wide viewport, showing the ring and legend pushed to opposite edges with a large empty strip between them, plus: "i still would like to close this gap, maybe makign the text bigger or somthing?"

**Response:** `.donut-row`'s `justify-content: space-between` (from UI feedback pass six) anchored the ring to the card's left edge and the legend to its right, reasoning the card's own width would "fill the gap between them" the way the bar chart's bars stretch to fill theirs — that held at the widths tested at the time, but neither the ring (fixed pixel size) nor the legend (content-sized text) actually grows to fill a wide card, so the "gap between them" was just empty space. Switched to `justify-content: center`, treating the ring+legend as one unit so it centres with symmetric breathing room on both sides instead of one lopsided dead strip — that's the actual fix, independent of viewport width. Also sized up the legend per the user's own suggestion (font-size 0.85rem → 1.05rem, swatch 0.7rem → 0.9rem, more row/gap spacing) so the centred group carries more visual weight too. Deliberately left the ring's own pixel size and the punched-centre text alone — both are already sized to fit a fixed-diameter circle rendered at every viewport including phone width, so growing them risked overflow or squeezing the mobile legend (Feedback Sprint 2's own item 10) for no clear benefit, when the legend had no such ceiling to grow into.

**Verified before accepting:** `tsc -b --force` and `eslint src`, both run directly on the user's machine via the device bridge, passed clean.

**Not yet done:** the user's own look at the fix on a real wide screen — this is exactly the kind of change that reads differently live than reasoned about from CSS source, per this project's own established caution about visual changes. `git push origin main` for this commit.

## 23/8/26 (Feedback Sprint 3 — five items)

**Prompt:** "alright lets do the same thing as feedback sprint 2 but now its feedback sprint 3. here are the new additions. rember to uopafte all the relevent docs and commit after every change. if you have any questions please ask," followed by a five-item list: (1) enable 2FA; (2) clicking a dashboard stat tile should also highlight the matching pie/donut slice; (3) make the profile clickable; (4) a first-time onboarding tour with arrows pointing users to where things are; (5) clicking the logo should open an about page.

**Response:** asked four clarifying questions up front, as invited — 2FA method (TOTP authenticator app), 2FA scope (opt-in from a settings page vs mandatory), what clicking the profile block should do (a dedicated Profile/Settings page vs a dropdown menu), and how to build the tutorial (a custom overlay vs a library) — the user picked the recommended option each time. Built in dependency order rather than the list's order — item 3 (profile page) and item 5 (about page) first, since item 1's two-factor settings needed somewhere to live — while keeping each commit's `(N/5)` label matching the item's original position, so the commit history still maps onto what was asked. Five independent changes, committed one at a time per the user's explicit instruction, with docs updated at the end as its own step (this entry) rather than per-item, since several of them (the security review, the iteration log, this file) read better as one coherent sprint-level pass — see the five `Feedback Sprint 3 (N/5)` commits in git log for each individual diff.

1. **Profile made clickable (3/5)** — the sidebar's profile block, previously a decorative `<div>`, is now a real `<button>` opening a new `ProfilePage.tsx` (derived name/initials and join date, pulled the name/initials logic out into a shared `lib/userDisplay.ts` so `Sidebar.tsx` and `ProfilePage.tsx` can't drift). `App.tsx`'s `View` type and Sidebar's `onNavigate` type both widened to add `'profile'`.
2. **About page from the logo (5/5)** — the sidebar's logo/wordmark, also previously a decorative `<div>`, is now a real `<button>` opening a new `AboutPage.tsx` with static content: what the app does, and a plain-language explanation of the unsafe-save-blocking behaviour from Feedback Sprint 2. Same `View`/`onNavigate` widening, this time adding `'about'`.
3. **Two-factor authentication (1/5)** — opt-in TOTP MFA, built entirely on Supabase Auth's own MFA API (`enroll`/`challenge`/`verify`/`unenroll`/`listFactors`) rather than a custom implementation — no new table, no secret ever touches this app's own database. `ProfilePage.tsx` grew a `TwoFactorSettings` section: enrolling shows a QR code and manual-entry secret and requires one correct code before the factor counts as verified; disabling requires a fresh code too, so a bare signed-in session can't turn it off by itself. A new `MfaChallenge.tsx` is the sign-in-time half — `App.tsx`'s top-level `App()` calls `getAuthenticatorAssuranceLevel()` after every session change and shows it instead of the authenticated app when the session hasn't reached `aal2` but a verified factor says it needs to. One real TypeScript snag: Supabase's own SDK types claim `listFactors()`'s per-type arrays (`data.totp`) only ever hold `'verified'` factors, which isn't true at runtime — finding stray unverified factors from an abandoned enrollment attempt (which need clearing before a fresh enrollment) had to go through the untyped `data.all` array instead, filtered by hand.
4. **Stat tile clicks highlight the matching donut slice (2/5)** — `Donut.tsx` gained an optional `activeKey` prop, merged with its existing hover state (`hovered ?? activeKey`) so a live hover always wins but the selected tile's slice stays highlighted once the mouse leaves, instead of the ring going back to its plain total. `Dashboard.tsx` passes `activeKey={selected === 'all' ? null : selected}` to the status donut. `PlaceDashboard.tsx` has no donut of its own, so this item only touches the main `Dashboard`.
5. **First-time onboarding tour (4/5)** — a custom-built overlay (`OnboardingTour.tsx`), not a new dependency — uses `framer-motion`, already a dependency for the modal/donut animations. Walks 5 fixed steps, each an arrowed tooltip pointing at a `data-tour="..."` anchored element (sidebar nav, the dashboard's stat-tile row, the add-item FAB, the sidebar profile block, the sidebar logo), dimming the rest of the page with an oversized `box-shadow` spread rather than an SVG mask. Shown once per account — checked via the account's own `session.user.user_metadata.has_seen_tutorial`, not `localStorage`, since "the first time a user opens the app" should follow the account rather than the browser (the same reasoning behind making 2FA opt-in from a settings page rather than a device flag). Completing or skipping persists that flag via `supabase.auth.updateUser`.

Two ESLint findings caught along the way, both from `eslint-plugin-react-hooks`'s newer `set-state-in-effect` rule: an initial version of the MFA-gate effect and the two-factor status-check effect both called a state setter synchronously in the effect body (or via a function statically traced back to one). Fixed by restructuring rather than suppressing — the MFA gate's "reset to checking on every session change" was replaced with a value *derived* at render time (tagging the last completed check with the account it was for, rather than resetting state proactively), which also fixed a latent UX issue: the original version would have flashed the whole app back to a loading screen on every plain token refresh, not just a real sign-in.
...
## 25/8/26 (Sign-up confirmation emails: diagnosis and custom-SMTP fix)

**Prompt:** two screenshots of the live sign-up form showing `email rate limit exceeded`, with: "A user tried to sign in but didnt get an email and got this error message. their uid didnt come up on the database either. please help me fix"

**Response:** read `Auth.tsx`, `supabase/config.toml` and the existing docs before answering, then checked Supabase's current SMTP documentation rather than relying on recalled limits. Two stacked causes, only one of which was visible: the built-in email service is capped at 2 emails/hour project-wide (the visible error), and it only delivers to pre-authorised team-member addresses (the invisible one, and the more important, since an outside tester's email was never going to arrive). The missing uid was explained rather than treated as a second bug: Supabase Auth returns the 429 before writing the row, so no account existed to find. Two clarifying questions asked before building: which fix route (custom SMTP vs turning off email confirmation vs both), and whether to fix the error-message handling in the app as well. Both answered with the recommended option: custom SMTP, and yes to friendly errors.

1. **`src/lib/authErrorMessage.ts` (new)**: a pure function mapping GoTrue error strings to plain English, matched on lowercased substrings rather than exact strings, since supabase-js surfaces `error.message` and those strings carry variable detail and get reworded between versions. Covers the rate limit, the per-address cooldown (preserving the retry seconds), invalid credentials, unconfirmed email, already-registered, short password, malformed address, disabled sign-ups, and network failure. Anything else falls through to the raw message, deliberately, so a new failure mode is still visible in the UI.
2. **`Auth.tsx`**: both the sign-up and sign-in branches now render `authErrorMessage(error.message)` instead of the raw string. The rate-limit wording states explicitly that the account was **not** created, which was the actual missing information: the tester couldn't tell whether to retry, wait, or try signing in. The success message now also points at the spam folder.
3. **`docs/smtp-setup.md` (new)**: a click-by-click runbook for the manual half: creating a Brevo account (free tier, 300/day, sends to arbitrary recipients from a verified individual sender with no domain required, unlike Resend), verifying a sender, generating an SMTP *key* rather than an API key, filling Supabase's Auth → Emails → SMTP Settings, raising the email rate limit afterwards (locked at 2/hour until custom SMTP is on), the long-outstanding Redirect URLs check, unblocking the affected tester via Auth → Users → Add user with Auto Confirm, and how to verify with a non-team-member address.
...
## 28/8/26 (Fleshing out the profile page, moving the theme control onto it)

**Prompt:** flesh out the profile page, add more to fill it up and make it look nice, move the light/dark mode toggle to it, and update all the relevant docs.

**Response:** read `ProfilePage.tsx`, `Sidebar.tsx`, `App.tsx`, `useTheme.ts`, `OnboardingTour.tsx`, `useExpiryNotifications.ts` and the profile-related CSS before proposing anything, then asked two clarifying questions rather than guessing at "more stuff": which sections to add, and whether "move" meant removing the sidebar's toggle outright. The user chose Appearance, Notifications and Account actions, and yes to removing it. A fourth option, a "kitchen at a glance" stats block, was offered and not taken; it was not built, since the dashboard already answers that question.

1. **`src/components/ProfilePage.tsx` (rewritten)**: an identity header (gradient banner, overlapping avatar, and a Member since / Two-factor / Theme summary strip) above four cards, all rendered through one new local `SettingsSection` shell so they share a single layout rather than four hand-built ones. `TwoFactorSettings` lost its own card wrapper and heading to that shell, and now reports its settled on/off state up through an `onStatusChange` callback so the header strip can show it without a second `listFactors()` call. New `NotificationSettings` reads `Notification.permission` directly rather than caching a flag, so it cannot disagree with the browser; it shows no button in the `denied` state, since `requestPermission()` resolves as denied without prompting there, and offers a test notification once granted.
2. **`src/hooks/useTheme.ts`**: exposes `setTheme` alongside the existing `toggleTheme`. The Appearance section is a two-option radiogroup where clicking the already-active theme must do nothing, which a toggle cannot express; `toggleTheme` stays for the sign-in screen, the one place with no profile page to reach.
3. **`src/components/Sidebar.tsx`**: theme button and both theme props removed, leaving Sign out alone in the footer.
4. **`src/App.tsx`**: threads `theme`/`setTheme` to `ProfilePage` instead of `Sidebar`, and adds `replayTour()`, which navigates to the dashboard *before* showing the tour, because `OnboardingTour` drops steps whose anchors are not mounted and would otherwise have run a silently shortened tour when replayed from the profile page.
5. **`src/App.css`**: the profile block rewritten for the four-card layout, plus a `.settings-pill` state chip shared by the notification and two-factor states, the segmented theme control, the account action rows, and phone-width behaviour.
6. **Docs**: `docs/decisions.md` (new "Profile page build-out" section), `docs/09-iteration-log.md`, `docs/02-requirements.md` (theme requirement rewritten, four new requirements for the profile page and the notification permission UI), `docs/06-front-end-architecture.md` (component tree and the theme paragraph), `README.md`, and `CLAUDE.md`. Two stale "Known defect" notes in `02-requirements.md`, covering the About page copy and the onboarding tour's step counter, were also removed after confirming in the source that the 27/8/26 sweep had genuinely fixed both.

**Verified before accepting:** `tsc -b --force`, `eslint src`, `vite build` and `npm test` (161/161) all clean, run directly on the user's machine via the device bridge. Then a disposable Playwright harness rendering the page's real markup against the project's actual `index.css`/`App.css` at 1440x900 and 390x844 in both themes, measuring the computed layout rather than only screenshotting it. That caught two real bugs the four checks above could not: the name block is almost exactly as tall as the avatar, so aligning their bottoms with `align-items: flex-end` dragged the name onto the indigo banner as near-black text; and the selected theme option's `--color-surface` fill is lighter than its track in light mode but darker in dark mode, so it read as recessed rather than selected. Both fixed and re-verified. Note that `npm run build` cannot be used as-is from this session: Vite empties `dist/` first and the device bridge cannot unlink files, so the build was run with `--outDir` pointed at scratch space outside the mounted folder.

**Not yet done:** the user's own look at the page in the running app, particularly the browser permission prompt and the test notification, which a static harness cannot exercise. Not committed: left in the working tree for review, per the review-before-commit process adopted in Feedback Sprint 4. Still outstanding from earlier sessions and unchanged by this one: running the `20260827000000` migration against the live Supabase project, and rotating the database password.

## 28/8/26 (Dashboard charts: removing the bar axis, filling both chart cards)

**Prompt:** with two screenshots of the running dashboard — remove the line in the bar chart and fill up its open space; make the ring chart and its key take up more space within their card; do not change the size of the cards themselves.

**Response:** measured before changing anything. Built a harness rendering the two chart cards' real markup against the project's actual `index.css`/`App.css` and read the computed layout at four viewports first. That reframed both asks: the bar chart's unused space was horizontal, not vertical (a 46px bar cap inside a 125-158px column), and the ring's card is 240px tall at 1440x700 but 430px at 1920x1080, so the reason it never filled its card was that a fixed pixel diameter cannot suit a card whose height varies by 190px.

1. **`src/components/Donut.tsx` + `src/App.css`**: `size` became a maximum rather than a diameter. It is passed to CSS as a `--donut-size` custom property; `.donut-shape` takes it as a width, holds `aspect-ratio: 1`, and clamps with `max-width`/`max-height` against what the card and legend leave. The `<svg>` fills that box and the punched hole is sized from it, so the ring scales as one piece.
2. **`src/components/Dashboard.tsx`**: the ring's value raised 168 → 220, now an upper bound. Measured result: 220px where the card holds it, 192px at 1280x800, 170px at 1440x700, none overflowing.
3. **`src/App.css` (bar chart)**: the baseline rule removed outright rather than restyled — every bar starts from the same edge of the same track, so it marked a boundary the bars already draw. The track went from `max-width: 46px` to `width: 72%; max-width: 110px`, a percentage so bars stay proportional as the card narrows; value and label type sized up to match.
4. **`src/App.css` (key)**: legend to 1.15rem with larger swatches and row spacing, plus a 200px minimum width so its counts line up as a column, reset at phone width.
5. **Docs**: `docs/decisions.md` (new "Dashboard charts: filling their cards"), `docs/09-iteration-log.md`, `docs/02-requirements.md` (a new requirement that both charts size to their card rather than to fixed dimensions, and that the bar chart carries no baseline rule), and `CLAUDE.md`.

**Three bugs the measurements caught that a single screenshot would not have:** `.donut-row` had no `min-height: 0`, so it refused to shrink below the ring's content size and overflowed its card by 38px at 1440x700 — and it is also what gives `.donut-shape`'s `max-height` something smaller to clamp against, so the two only work as a pair. `aspect-ratio` only re-derives the axis it was not given, so a `max-height` clamp left the shape at 220x170 on a short viewport; the `<svg>` letterboxes and stays circular, but a hole sized in plain percentages of that box would have been a 132x102 ellipse inside a 170px ring, fixed by making the shape a size container and sizing the hole in `cqmin`. And the key's new minimum width pushed the charts column wider than the attention card at 1280px, since a `1fr` grid track grows to its content unless given `min-width: 0` — a direct violation of the "don't change the cards' size" part of the brief.

**Verified before accepting:** `tsc -b --force`, `eslint src`, `vite build` and `npm test` (161/161) all clean, run on the user's machine via the device bridge. Then the harness at eight viewports (390x844, 768x1024, 1280x800, 1366x640, 1440x700, 1440x900, 1536x960, 1920x1080) asserting on measured values rather than appearance: no row overflowing its card, both chart cards equal in width, no horizontal page scroll, a computed `border-bottom-width` of 0 on the bar chart, and the hole circular, concentric with the ring, and 0.600 of its drawn diameter at every size.

**Not yet done:** the user's own look at it in the running app. Not committed: left in the working tree alongside the profile page build-out, per the review-before-commit process.
