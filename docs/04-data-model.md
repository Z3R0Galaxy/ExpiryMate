...
| `supabase/migrations/20260818000000_split_items_rls_policies.sql` | Replaces the single `for all` ownership policy with four per-operation policies (select/insert/update/delete), each still enforcing `auth.uid() = user_id` — see `05-security-review.md` and `decisions.md`, "Marking-alignment review" |
| `supabase/migrations/20260818010000_add_items_index.sql` | Adds the composite index `items_user_id_expiry_date_idx` on `(user_id, expiry_date)`, matching the one access pattern `useItems.fetchItems` actually issues |
...
| `supabase/migrations/20260827000000_widen_date_opened_check.sql` | Replaces the auto-named `items_date_opened_check` with `items_date_opened_not_future`, widened from `date_opened <= current_date` to `current_date + 1`. `current_date` is evaluated in the server's UTC timezone while the app validates against the browser's local date, so in any timezone ahead of UTC a legitimate "opened today" was refused by Postgres for the first hours of every day. One day of tolerance covers every real offset while still rejecting a genuinely future date — see the 27/8/26 sweep report, finding 2 |
...
The full set replays cleanly in order: verified on 27/8/26 by applying all eight files to a fresh Postgres 16 instance running in UTC and comparing the resulting schema, enum values, constraints, indexes and policies against this document. See `tests/integration/README.md` for what that run confirmed.
